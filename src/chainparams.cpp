...
public:
    CMainParams() {
        ...
        vSeeds.emplace_back("seed.feritcoin.sipa.be"); // Pieter Wuille, only supports x1, x5, x9, and xd
        vSeeds.emplace_back("dnsseed.bluematt.me"); // Matt Corallo, only supports x9
        vSeeds.emplace_back("dnsseed.feritcoin.dashjr.org"); // Luke Dashjr
        vSeeds.emplace_back("seed.feritcoinstats.com"); // Christian Decker, supports x1 - xf
        vSeeds.emplace_back("seed.feritcoin.jonasschnelli.ch"); // Jonas Schnelli, only supports x1, x5, x9, and xd
        vSeeds.emplace_back("seed.btc.petertodd.org"); // Peter Todd, only supports x1, x5, x9, and xd

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,95);  //95 stand for f (as feritcoin) according to https://en.bitcoin.it/wiki/List_of_address_prefixes
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,122);  //as r
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,33);  //3 as E
...
public:
    CTestNetParams() {
        ...
        for(; consensus.powLimit < genesis.GetHash(); genesis.nNonce++){}
            printf("New genesis merkle rot: %s\n", genesis.hashMerkleRoot.ToString().c_str());
