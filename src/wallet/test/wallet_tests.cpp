...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 FRT in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

        BOOST_CHECK( testWallet.SelectCoinsMinConf(195 * CENT, 1, 1, 0, vCoins, setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, 2 * COIN);  // we should get 2 FRT in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see feritcoind log for details) and could be dealt with by "
