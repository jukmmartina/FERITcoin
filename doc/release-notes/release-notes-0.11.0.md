Feritcoin Core version 0.11.0 is now available from:

  <https://feritcoin.org/bin/feritcoin-core-0.11.0/>
...
For example, add the following to `feritcoin.conf`:
...
related to the blockchain in the feritcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Feritcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Feritcoin Core will 
...
For an example script see [feritcoin-submittx](https://github.com/laanwj/feritcoin-submittx).
...
- #5334 `cf87536` libferitcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new feritcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FeritcoinAmountField
- #5753 `0f44672` Add feritcoin logo to about screen
...
- #5809 `46bfbe7` Add feritcoin-cli man page
...
- #5849 `41259ca` contrib/init/feritcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Feritcoin-Qt.app to Feritcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for feritcoin-tx)
