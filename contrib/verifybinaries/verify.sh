...
###   feritcoincore.org and feritcoin.org and compares them.
...
WORKINGDIR="/tmp/feritcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://feritcoincore.org"
HOST2="https://feritcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="feritcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'feritcoin-',
...
if [ $? -ne 0 ]; then
   echo "feritcoin.org failed to provide signature file, but feritcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "feritcoin.org and feritcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Feritcoin Core binary release signing key installed?"
   fi
