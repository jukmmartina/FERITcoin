...
The RPM spec file provided here is for Feritcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://feritcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Feritcoin git repository](https://github.com/feritcoin/feritcoin) but are not part of
...
Currently only `Source10` is used, for the example `feritcoin.conf` file.
...
in the feritcoin git repository. At present only an SVG version of the Feritcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the feritcoin
...
needed when 0.12.1 is released, a proper fix is already in the Feritcoin git
master branch.
...
idea to build Feritcoin Core against a static version of that library compiled
...
The `feritcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `feritcoin-server` package. At this point, most RPM based
...
The `feritcoin-server` package should have SELinux support. How to properly do
...
Feritcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/feritcoin/). He has been
packaging Feritcoin for Fedora at least since 2012.
...
4. I renamed the `feritcoin` package that contains the Qt GUI to `feritcoin-core` as
that appears to be how the general population refers to it, in contrast to
`feritcoin-xt` or `feritcoin-classic`. I wanted to make sure the general population
