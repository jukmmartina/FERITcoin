#!/bin/bash
# Copyright (c) 2013 The Feritcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Feritcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Feritcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Feritcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Feritcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Feritcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Feritcoin 0.8 datadir."
    ;;
esac
